...
- **Data Inspector** — 선택된 View의 `data`, `_args`, 관계(parent/children), 메타 정보 표시
...
- **DOM Highlight** — 호스트 앱 위에서 마우스 hover로 View 경계 표시 + 클릭으로 선택
- **Timeline** — 모든 이벤트 통합 시간축, 유형별 필터

## 설치

```bash
pnpm add -D @rune-ts/devtools vite-plugin-rune-devtools
```
...
if (import.meta.env.DEV) {
  const { initDevtools } = await import('@rune-ts/devtools');
  initDevtools();
}
```
...
import type { DevtoolsPlugin } from '@rune-ts/devtools';

const myPlugin: DevtoolsPlugin = {
  name: 'my-plugin',
  setup(api) {
    api.onViewRender((snapshot) => { /* ... */ });
    api.onRedraw((record) => { /* ... */ });
    api.onListViewMutation((record) => { /* ... */ });
    api.addTimelineEvent({ type: 'render', viewId: 'v1', ... });
  },
};
...
| 패키지 | 설명 |
|--------|------|
| `@rune-ts/devtools` | 코어 — 인터셉터, 스토어, UI |
| `vite-plugin-rune-devtools` | Vite 플러그인 — dev 모드에서 자동 주입 |
